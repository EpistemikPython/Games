...
FPS = 50 # needs to be evenly divisible by 1000 or delay timing error accumulates
BOARD_LENGTH = 4 # i.e. 4x4 tiles game
TILE_SIZE = 150 # pixels per side
GAP_SIZE = 10
MARGIN = 20
FRAME_WIDTH  = BOARD_LENGTH*TILE_SIZE + (BOARD_LENGTH+1)*GAP_SIZE + 2*MARGIN
FRAME_HEIGHT = BOARD_LENGTH*TILE_SIZE + (BOARD_LENGTH+1)*GAP_SIZE + 4*MARGIN
...
def draw_tile(screen, value, x, y):
    color = TILE_COLORS.get(value, (66, 54, 42))
    rect = pygame.Rect(x, y, TILE_SIZE, TILE_SIZE)
    pygame.draw.rect(screen, color, rect)
    if value != 0:
        text = gfont.render(str(value), True, (0, 0, 0))
...
def add_new_tile(board):
    empty_tiles = [(r, c) for r in range(BOARD_LENGTH) for c in range(BOARD_LENGTH) if board[r][c] == 0]
    if empty_tiles:
        row, col = random.choice(empty_tiles)
        board[row][col] = 2 if random.random() < 0.85 else 4

def slide_row_left(row):
    new_row = [i for i in row if i != 0]
    new_row += [0] * (BOARD_LENGTH-len(new_row))
...
def run():
    ...
    while running:
        ...
        # game sync & time display
        delay = clock.tick(FPS)
        # pause the timer when the game is minimized or over
        if won or lost or not pygame.display.get_active():
            delay_time += delay
        timetext = gfont.render(f"Time: {(pygame.time.get_ticks() - delay_time)//1000}", True, (0, 127, 0))
        ...
        if won:
            wintext = gfont.render("You WIN!", True, (252, 10, 10))
            wtext_rect = wintext.get_rect(center=(FRAME_WIDTH//2, FRAME_HEIGHT//2))
            frame.blit(wintext, wtext_rect)
        elif lost:
            lostext = gfont.render("You LOSE :(", True, (225, 20, 20))
            ltext_rect = lostext.get_rect(center=(FRAME_WIDTH//2, FRAME_HEIGHT//2))
            frame.blit(lostext, ltext_rect)
